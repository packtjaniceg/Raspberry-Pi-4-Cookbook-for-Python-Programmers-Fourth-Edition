...
def determine_dir(the_ball, object):
    global delta_x, delta_y
    if the_ball[xTOP] == object[xBTM] or the_ball[xBTM] == object[xTOP]:
        delta_x = -delta_x
    elif the_ball[yTOP] == object[yBTM] or the_ball[yBTM] == object[yTOP]:
        delta_y = -delta_y
...
while RUNNING:
    ...
    for objt in objects:
        if objt != ball:
            if not dir_changed:
                determine_dir(canv.coords(ball), canv.coords(objt))
                dir_changed = True
            if brick[0] <= objt <= brick[len(brick)-1]:
                canv.delete(objt)
            if objt == bottom:
                text = canv.create_text(300, 100,
